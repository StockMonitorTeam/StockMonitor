...
trait TelegramService extends TelegramBot
  with Polling
  with Commands { self : DataStorage =>

  val token: String = ConfigFactory.load().getString("StockMonitor.Telegram.apitoken")

  logger.info("starting telegram bot")


  private val activeUsers: mutable.Map[Long, ActorRef] = mutable.HashMap.empty
  ...
  onMessage { msg =>
    logger.info(s"message from ${msg.chat.firstName.getOrElse("")} : ${msg.text.getOrElse("")}")
    for {user <- activeUsers.get(msg.chat.id)
